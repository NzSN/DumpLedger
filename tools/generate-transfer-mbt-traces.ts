...
function locateRepositoryRoot(moduleDirectory: string): string {
  ...
  const found = candidates.find((candidate) =>
    existsSync(join(candidate, "specs", "DumpLedgerTransfer.tla")));
  if (found === undefined) {
...
const fixturesRoot = join(repositoryRoot, "test", "fixtures", "mbt", "transfer-traces");
...
function encodedSet(state: JsonObject, name: string): string[] {
  const value = state[name];
  if (!isObject(value)) return [];
  const members = value["#set"];
  if (!Array.isArray(members)) return [];
  return members.map((member) =>
    isObject(member) && typeof member["#bigint"] === "string"
      ? member["#bigint"]
      : "?");
}
...
function bundleField(state: JsonObject, field: string): Json | undefined {
  const bundle = state["bundle"];
  return isObject(bundle) ? bundle[field] : undefined;
}

function terminalAction(state: JsonObject, action: string): boolean {
  return state["action_taken"] === action;
}

function importFinished(state: JsonObject): boolean {
  return terminalAction(state, "ImportFinish") &&
    bundleField(state, "status") === "finished";
}
...
const traceCases: readonly TraceCase[] = [
  ...
  },
  {
    ...
    witnessModule: "TransferTamperReject",
    lengthBound: 20,
    terminalDescription: "ImportFinish with the tampered dump slot 1 rejected transfer-failed",
    terminalMatches: (state) =>
      importFinished(state) &&
      sequenceSlot(state, "dumpPhase", 1) === "rejected" &&
      sequenceSlot(state, "validation", 1) === "transfer-failed",
  },
  {
    destination: "03-tombstones.itf.json",
    witnessModule: "TransferTombstones",
    lengthBound: 28,
    terminalDescription: "ImportFinish with deleted and rejected tombstones carrying recorded evidence",
    terminalMatches: (state) =>
      importFinished(state) &&
      sequenceSlot(state, "dumpPhase", 1) === "deleted" &&
      sequenceSlot(state, "dumpPhase", 2) === "rejected" &&
      sequenceSlot(state, "coverage", 1) === "partial" &&
      sequenceSlot(state, "validation", 1) === "valid" &&
      sequenceSlot(state, "validation", 2) === "invalid",
  },
  {
    destination: "04-fingerprint-mismatch.itf.json",
    witnessModule: "TransferFingerprintMismatch",
    lengthBound: 24,
    terminalDescription: "ImportFinish with the issued grant revoked under a mismatched fingerprint",
    terminalMatches: (state) =>
      importFinished(state) &&
      state["fingerprintMatches"] === false &&
      sequenceSlot(state, "tokenState", 1) === "revoked" &&
      sequenceSlot(state, "tokenState", 2) === "consumed" &&
      sequenceSlot(state, "dumpPhase", 2) === "available",
  },
  {
    destination: "05-export-failed-deleted.itf.json",
    witnessModule: "TransferExportFailDelete",
    lengthBound: 12,
    terminalDescription: "DeleteBundle after a failed export, ledger untouched",
    terminalMatches: (state) =>
      terminalAction(state, "DeleteBundle") &&
      bundleField(state, "status") === "absent" &&
      sequenceSlot(state, "dumpPhase", 1) === "available",
  },
  {
    destination: "06-import-hard-failed.itf.json",
    witnessModule: "TransferImportHardFail",
    lengthBound: 18,
    terminalDescription: "ImportHardFail with the declared dump still pending",
    terminalMatches: (state) =>
      terminalAction(state, "ImportHardFail") &&
      bundleField(state, "status") === "import-failed" &&
      sequenceSlot(state, "dumpPhase", 1) === "absent" &&
      encodedSet(state, "digestRecorded").length === 0 &&
      sequenceSlot(state, "tokenState", 1) === "consumed",
  },
  {
    destination: "07-skipped-in-flight.itf.json",
    witnessModule: "TransferSkippedInFlight",
    lengthBound: 24,
...
async function run(): Promise<void> {
  ...
  const generationRoot = await mkdtemp(join(tmpdir(), "dump-ledger-transfer-mbt-"));
  let allCurrent = true;
  try {
