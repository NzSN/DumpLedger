...
test("a second transfer job conflicts (409) while one is running", async t => {
  ...
  const { server, cleanup } = await makeFixture(engine => ({
    ...
    findSymbolArtifact: (name, id, kind) => engine.findSymbolArtifact(name, id, kind),
    findSymbolArtifactByStorePath: (name, id) => engine.findSymbolArtifactByStorePath(name, id),
    listSymbolArtifacts: () => engine.listSymbolArtifacts(),
